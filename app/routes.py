...
def index():
    ...
    next_url = url_for('index', post=posts.has_next) if posts.has_next else None
    prev_url = url_for('index', post=posts.has_prev) if posts.has_prev else None
    return render_template('index.html', title='Home', form=form, posts=posts.items, next_url=next_url, prev_url=prev_url)


@app.route('/login', methods=['GET', 'POST'])
def login():
    if current_user.is_authenticated:  # user that is logged in already
        return redirect(url_for('index'))
